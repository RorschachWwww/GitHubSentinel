...
def hacker_news_job(hacker_news_client, report_generator, notifier, days):
    LOG.info("[开始执行定时任务]")
    # 遍历每个订阅的仓库，执行以下操作
    markdown_file_path = hacker_news_client.export_daily_progress()
    # 从Markdown文件自动生成进展简报
    report, report_file_path = report_generator.generate_daily_report(markdown_file_path)
    notifier.notify("hacker_news", report)
...
def main():
    ...
    llm = LLM(source="hacker_news")  # 创建语言模型实例
    report_generator = ReportGenerator(llm,"hacker_news")  # 创建报告生成器实例

    # 启动时立即执行（如不需要可注释）
    hacker_news_job(hacker_news_client, report_generator, notifier, config.freq_days)

    # 安排每天的定时任务
    schedule.every(config.freq_days).days.at(
        config.exec_time
    ).do(hacker_news_job, hacker_news_client, report_generator, notifier, config.freq_days)
