...
def generate_stack_overflow_report(model_type, model_name):
    config.llm_model_type = model_type
...
with gr.Blocks(title="GitHubSentinel") as demo:
    ...
    with gr.Tab("Hacker News 热点话题"):
        ...
        # 创建 Hacker News 热点话题 Tab
    with gr.Tab("Stack Overflow python热点问题"):
        gr.Markdown("## Stack Overflow python热点问题")  # 添加小标题

        # 创建 Radio 组件
        model_type = gr.Radio(["openai"], label="模型类型",
        ...
        # 将按钮点击事件与导出函数绑定
        button.click(generate_stack_overflow_report, inputs=[model_type, model_name, ], outputs=[markdown_output, file_output])
